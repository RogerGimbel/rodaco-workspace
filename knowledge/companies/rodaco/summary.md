# Rodaco

## Current Status
- **Phase**: MVP Development
- **Team Size**: 3 people

## Team
| Name | Role | Telegram |
|------|------|----------|
| [[Roger Gimbel]] | Developer | @roger_gimbel |
| [[Dale Abbott]] | Finance/Ideas | @DMA5400 |
| [[Stuart Seligman]] | Ideas/Marketing | @Sgseligman |

## Focus
- Web and mobile app development
- Primary product: [[BeerPair]] (live at beerpair.com)
- B2B automation: [[Ocean One Marine]]

## Tech Stack
- Frontend: React/Vite
- Backend: Supabase
- Mobile: WebView wrapper (Despia)
- Hosting: Vercel, Cloudflare

## Infrastructure
- [[Intel MacBook]] — OpenClaw server (headless)
- [[Raspberry Pi]] — Media stack (Jellyfin, *arr suite)
- [[M5 MacBook]] — Roger's daily driver

## Communication
- Primary: Telegram group
- AI Assistant: OpenClaw (Rodaco)
- Dashboard: [[Mission Control]] (http://100.124.209.59:3333)
- Knowledge: [[Obsidian]] vault synced via iCloud

## Decision Making
- Technical: [[Roger Gimbel]] (with input)
- Financial: [[Dale Abbott]] (with input)
- Product/Marketing: [[Stuart Seligman]] (with input)
- Major pivots: All three agree

## History
- [2026-01-29] OpenClaw integration complete for team collaboration
- [2026-02-01] [[BeerPair]] web app live
- [2026-02-11] Mission Control v2 shipped
- [2026-02-11] Obsidian vault connected via iCloud
- [2026-02-12] BeerPair B2B GTM sprint — 17 assets built (pricing, sales funnel, pitch deck, etc.)
- [2026-02-13] Mission Control v3 started — API-first architecture with 35 endpoints
- [2026-02-14] SOPS/age secrets hardening deployed on Intel MacBook
- [2026-02-15] Mission Control v3 — 5 new metrics endpoints, cron watchdog for auto-recovery
