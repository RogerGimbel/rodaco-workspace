...
- **Gemini 3.1 Pro Preview** — announced 2026-02-19. Model ID: `gemini-3.1-pro-preview`. Confirmed in our API key's model list. 503 on launch day (high traffic). **Cron polling every 30 min** (`gemini-3.1-availability-check`, ID: `61f6cfa2`). Will Telegram-notify Roger + self-destruct when live. Benchmarks: 94.1% GPQA Diamond, 77.1% ARC-AGI-2. Pricing: $2/$12/M input/output — similar to Sonnet. **Roger wants to test as daily driver once available.**
